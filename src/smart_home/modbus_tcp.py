"""Generic raw Modbus-TCP reader (stdlib only) — fallback path.

This is a minimal Modbus-TCP client over a raw socket (function 0x03, read holding
registers), with wire framing and value decoding separated from socket I/O so they are
testable without hardware.

NOTE: the validated path for this project is the inverter's built-in WLAN on port **6607**,
which uses a proprietary handshake this raw client does NOT speak — see :mod:`smart_home.inverter`
(huawei-solar based). Keep this module for the **RS485-to-TCP bridge** or **SDongle port 502**
fallbacks, which speak standard Modbus-TCP.

Register addresses/types/gains are from the ``huawei-solar-lib`` definitions.
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not argv:
        print("usage: python -m smart_home.modbus_tcp <host-or-ip> [port]", file=sys.stderr)
        sys.exit(1)
    host = argv[0]
